...
- [x] `DFD-05` Production cutover targets near-zero downtime through blue/green
  web switching and database-backed single ownership for workers and providers.
...
- [ ] `DF1-09` Move the Python deployment to PostgreSQL before the runtime port.
  Pause ingress, checkpoint SQLite, export/import, verify the manifest, switch
  configuration, resume Python, and monitor.
...
- [ ] `DF2-03` Implement strict TypeScript equivalents of `TenantContext`,
  ...
  structured logging, and typed application errors.
- [ ] `DF2-04` Implement a PostgreSQL repository and transaction layer with
  request-scoped clients, typed row decoders, compound tenant lookups, and test
  transaction helpers. Routes and provider adapters may not access unscoped raw
  clients.
- [ ] `DF2-05` Preserve HMAC session, OAuth state, EventSub signature,
  constant-time comparison, CSRF/origin, credential encryption, token storage,
  and rotation semantics.
- [ ] `DF2-06` Prove Deno can read Python-created sessions and installation
  credentials. If encryption compatibility is impractical, implement an audited
  staged re-encryption migration before provider cutover.
- [ ] `DF2-07` Add role-selectable entry points for `web`, `jobs`, `analysis`,
  `discord`, and `twitch`, preserving `QBOT_ENABLED_SERVICES` behavior.
- [ ] `DF2-08` Add live and ready health checks for PostgreSQL, migration state,
...
- [ ] `DF3-01` Extract the application shell into Fresh layouts and components,
...
- [ ] `DF3-02` Port public, legal, health, Discord operator authentication, and
  community-switching routes.
- [ ] `DF3-03` Port overview, users, search, signals, and analytics as complete
  vertical slices with route, query service, policy guard, component, and tests.
- [ ] `DF3-04` Port moderation queues, review workspaces, rules, bulk actions,
  sanctions, and provider confirmation states.
- [ ] `DF3-05` Port commands, announcements, onboarding, integrations, settings,
  audit, and live operations.
- [ ] `DF3-06` Preserve current paths, methods, redirects, statuses, content
...
- [ ] `DF3-07` Port machine ingestion and Twitch EventSub separately from
  operator routes. Verify raw-body signatures before parsing and preserve size
  limits, replay protection, tenant resolution, capability checks, and response
  behavior.
- [ ] `DF3-08` Run Fresh in shadow-read mode against PostgreSQL. Mirror safe GET
  traffic and recorded or synthetic mutations, but never live provider actions.

### DF3 Exit Gate

- [ ] `DF3-G1` Every route in the frozen manifest has a Fresh implementation and
  a parity result.
- [ ] `DF3-G2` HTTP, auth, tenant-isolation, no-JavaScript, and browser suites
  pass on desktop and mobile.
- [ ] `DF3-G3` Shadow-read comparisons meet agreed correctness and latency
  thresholds with no unresolved severity-1 defects.
- [ ] `DF3-G4` Existing dashboard decomposition item `P4-01` is complete.

## DF4: Domain Services, Queues, and Scheduled Work

- [ ] `DF4-01` Port pure domain logic first: normalization, moderation rules,
  commands and templates, permissions, scoring, signals, analytics,
  intelligence, quotas, and SLO calculations.
- [ ] `DF4-02` Preserve model versions, thresholds, rounding, evidence,
  confidence, explanations, and deterministic outputs through golden fixtures.
- [ ] `DF4-03` Port observation ingestion and processing repositories while
  preserving unique keys, archive hashes, tenant ownership, idempotency,
  retries, leases, dead letters, and audit attribution.
- [ ] `DF4-04` Implement concurrent Deno workers with PostgreSQL transactional
  claims. Test crashes, recovery, starvation, duplicate execution, lease expiry,
  and tenant fairness before enabling live consumption.
- [ ] `DF4-05` Port maintenance, analytics, announcements, onboarding,
  notifications, retention, raw archive/replay, and backup orchestration.
- [ ] `DF4-06` Replace SQLite file backups with PostgreSQL-native backups or
...
- [ ] `DF4-07` Shadow deterministic analysis jobs without committing results.
  Transfer one live job type at a time through a PostgreSQL single-owner flag.

### DF4 Exit Gate

- [ ] `DF4-G1` Domain and job parity fixtures pass with no unexplained output
  differences.
- [ ] `DF4-G2` Fault and concurrency tests prove fair, idempotent processing and
  recovery after process termination.
- [ ] `DF4-G3` Deno owns all production job types and scheduled work while
  Python consumers remain disabled but available for rollback.

## DF5: Discord and Twitch Provider Port

- [ ] `DF5-01` Run compatibility spikes for maintained Deno-compatible Discord
  and Twitch libraries. Pin selected versions and retain recorded protocol
  fixtures. Use native WebSocket, `fetch`, or TCP/TLS behind provider interfaces
  where libraries cannot preserve required behavior.
- [ ] `DF5-02` Port Discord OAuth and installation flows, Gateway lifecycle,
...
- [ ] `DF5-03` Port Twitch broadcaster OAuth and token refresh, IRC lifecycle,
...
- [ ] `DF5-04` Preserve tenant and installation capability guards before every
  outbound action.
- [ ] `DF5-05` Add recorded protocol, fake-server, disconnect/resume,
  expired-token, signature/replay, revocation, and retry tests.
- [ ] `DF5-06` Transfer each live installation through a PostgreSQL ownership
  lease so only one runtime can receive events or perform actions for it.

### DF5 Exit Gate

- [ ] `DF5-G1` Recorded and fake-provider suites pass for both providers.
- [ ] `DF5-G2` Real non-production Discord and Twitch smoke tests pass for OAuth,
  installation, reconnect/resume, ingestion, commands, moderation,
...
- [ ] `DF5-G3` Deno owns all provider installations without duplicate events or
  actions. Python connectors remain disabled but rollback-ready.

## DF6: Packaging, Rollout, and Cutover

- [ ] `DF6-01` Update `install.sh`, `deploy/install.py`, systemd templates, and
  nginx templates for pinned Deno, explicit permission flags, separate process
  roles, Fresh static assets, graceful shutdown, migration gating, health
  checks, logs, and release-directory rollback.
- [ ] `DF6-02` Add Deno operational commands equivalent to `check-config`,
  `init-db` or migrate, `platform-audit`, `issue-pilot-invite`, `run`, and
  development watch.
- [ ] `DF6-03` Document PostgreSQL provisioning, migrations, import, backups,
  restore, credential rotation, provider ownership, cutover, and rollback.
- [ ] `DF6-04` Deploy Deno blue/green beside Python with Deno writes and provider
  actions disabled. Run parity, browser, SLO, migration, restore, security, and
  privacy checks.
...
- [ ] `DF6-06` Rehearse a successful cutover, an aborted cutover, nginx
  switchback, worker/provider lease handoff, and PostgreSQL restore. Record
  measured recovery time and data-loss bounds.
- [ ] `DF6-07` Execute cutover in dependency order: drain Python jobs and
  providers, acquire Deno ownership, verify queues, switch nginx, and verify
  health, auth, ingestion, moderation, jobs, and providers.
- [ ] `DF6-08` Monitor error rate, webhook acceptance, queue age, provider
  health, and tenant SLOs throughout the rollback window.
...
- [ ] `DF6-G3` Security and privacy review has no unresolved release blocker.
- [ ] `DF6-G4` Deno meets the current SLOs throughout the stabilization window.

## DF7: Stabilization and v1.0.0 Release

- [ ] `DF7-01` Remove Python source, Python tests, requirements files, virtualenv
  deployment paths, and SQLite production code after the rollback window.
- [ ] `DF7-02` Retain the SQLite importer as a supported offline migration tool.
- [ ] `DF7-03` Contract obsolete PostgreSQL columns only in a later migration
  after rollback compatibility is no longer required.
- [ ] `DF7-04` Update README, operations documentation, release notes, installer
  instructions, architecture diagrams, and support procedures for Deno/Fresh.
- [ ] `DF7-05` Record the final compatibility, accessibility, provider, rollout,
  restore, and rollback evidence in the release report.

### DF7 Exit Gate

- [ ] `DF7-G1` No production process or package depends on Python or SQLite.
...
- `src/__main__.py`: CLI and service lifecycle compatibility.
- `src/config.py`: environment, service selection, secrets, and safe summaries.
- `src/db.py`: migrations, schema, repositories, ingestion, and queues.
...
- `src/twitch.py`, `src/twitch_auth.py`, `src/twitch_eventsub.py`,
  `src/twitch_control.py`: Twitch behavior and security contracts.
- `src/intelligence/`: analytical and operational domain parity.
