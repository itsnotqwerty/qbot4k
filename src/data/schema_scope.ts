export interface ScopeRule {
  readonly scope: "global" | "organization" | "community" | "installation";
...
const GLOBAL_TABLES = [
  ...
  "derived_signal_windows",
  "derived_signals",
  "evaluation_labels",
  "external_feed_sources",
  "graph_metric_history",
  "graph_metrics",
  ...
  "model_registry",
  "observation_fts",
  "observation_fts_config",
  "observation_fts_content",
  "observation_fts_data",
  "observation_fts_docsize",
  "observation_fts_idx",
  "operational_metrics",
  "operator_accounts",
  "operator_discord_guild_permissions",
  "organizations",
  "platform_accounts",
  "processing_job_ownership",
  "raid_playbooks",
  "reputation_events",
  "saved_queries",
  "schema_migrations",
...
const COMMUNITY_TABLES = {
  ...
  processing_job_shadow_runs: ["processing_job_id", "processing_jobs"],
...
const INSTALLATION_TABLES = {
  community_announcement_deliveries: [
    "installation_id",
  ...
  discord_channels: ["guild_id", null],
  ...
  installation_runtime_leases: [
    "installation_id",
    "community_installations",
  ],
} as const;

const entries: Array<[string, ScopeRule]> = GLOBAL_TABLES.map((table) => [
  table,
  Object.freeze({ scope: "global" }),
]);
for (
  const [table, [ownerColumn, ownerTable]] of Object.entries(
    ORGANIZATION_TABLES,
  )
) {
  entries.push([
    table,
    Object.freeze({ scope: "organization", ownerColumn, ownerTable }),
  ]);
}
for (
  const [table, [ownerColumn, ownerTable]] of Object.entries(COMMUNITY_TABLES)
) {
  entries.push([
    table,
    Object.freeze({ scope: "community", ownerColumn, ownerTable }),
  ]);
}
for (
  const [table, [ownerColumn, ownerTable]] of Object.entries(
...
export const SCHEMA_SCOPE_INVENTORY: Readonly<Record<string, ScopeRule>> =
